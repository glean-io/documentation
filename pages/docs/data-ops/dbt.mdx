...
`source` has the same options as the corresponding field of [Glean data models](./config-schema/data-models.mdx). Unless otherwise specified, Glean automatically infers defaults for the source `physicalName` and `schema`. If all models use the same source, you can add the following to your dbt project configuration:
...
Any model without a `source` will then default to the configured `gleanDefaultSource`.
