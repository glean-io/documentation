...
Glean can import models directly from dbt by specifying metrics in your dbt yaml file. Glean will automatically sync dbt documentation and keep metrics in sync with downstream dashboards (whether they are checked into code or not).  It's also possible to integrate with CI/CD processes.

<Steps>

### Define connection information in your `dbt_project.yml`
...
models:
  +meta:
    ...
    glean-defaults:
      ...
      source:
        connectionName: Uploads # the name of the conneciton in Glean
        # can use jinja
        # schema is always optional
        schema: "{{ env_var('DBT_SCHEMA') }}"
```

### Define model information:

The simplest example of a Glean model just defines a metric.
...
- Glean will read the dbt manifest file and find all models with a `glean
- The name of dbt model is imported into Glean as the model name
