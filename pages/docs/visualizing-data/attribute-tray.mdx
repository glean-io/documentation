...
The attributes tray is the primary control for your chart data.
...
You can choose which attributes are visible in the attributes tray. There are two different ways to do this:
...
## Filtering from the attributes tray

Filters created in the attributes tray are applied across the entire page - filtering every other attribute, the numeric total, and the chart for the values selected in the attribute tile.

The default state for an attribute tile is this filtering mode. Checkboxes next to each attribute value indicate whether the item is included (shown in the view) or excluded (filtered out).

1. Hover over an attribute value.
2. To exclude the value, click it.
3. To include just that value, click the icon at the base of the value's metric bar.
